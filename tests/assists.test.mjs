...
test('sprint turns and braking keep a reachable ball without teleporting it',()=>{
 for(const team of [0,1]){
  ...
  // A ball knocked about 2 m ahead at a sprint is chased and trapped, which can take up to two more seconds.
  for(let i=0;i<240&&m.physics.ball.velocity.length()>=.15;i++){m.step(1/120,idle);assert.equal(m.owner,p);}
...
test('receiving assistance respects an away input in both attack directions and resets on kickoff',()=>{
 for(const team of [0,1]){const {m,p}=setup(team),dir=m.direction(team);m.owner=null;m.physics.reset(p.x+dir*4,0);m.receiving={player:p,expires:5};
  m.step(1/120,{axis:{x:-dir,z:0}});assert.equal(m.receiving?.player,p);assert.ok(p.vx*dir<0);
  m.receiving={player:p,expires:5};m.kickoff(team);assert.equal(m.receiving,null);
 }
});
