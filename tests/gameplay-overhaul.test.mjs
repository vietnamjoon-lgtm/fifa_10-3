...
test('the pressing defender takes a goal-side jockey point and brakes on arrival instead of charging the ball',()=>{for(const flip of [1,-1]){
 const {m,a,b}=setup();m.autoplay=true;if(flip<0)m.half=2;const dir=m.direction(a.team);Object.assign(b,{x:-dir*2,z:1,vx:-dir*4,vz:0,yaw:-dir*Math.PI/2});Object.assign(a,{x:-dir*9,z:0});m.owner=b;m.physics.reset(b.x-dir*.5,1);m.aiDefence=[{mode:'press',until:99},{mode:'press',until:99}];
 updateTeamAI(m);assert.equal(a.aiState,'PRESS');assert.ok((a.target.x-b.x)*dir<-.6,'target is between the carrier and our goal');assert.ok(Math.hypot(a.target.x-m.physics.ball.position.x,a.target.z-m.physics.ball.position.z)>.4,'not the ball itself');
 a.x=b.x-dir*2.2;a.z=1;updateTeamAI(m);assert.ok(a.speedCap>0&&a.speedCap<6.5,`arrives slow (${a.speedCap})`);}});
