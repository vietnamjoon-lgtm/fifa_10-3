...
test('preferred feet mirror logical contact and choose the reachable side',()=>{const {p}=match();p.yaw=0;const left=logicalFoot({...p,foot:'left'}),right=logicalFoot({...p,foot:'right'});assert.equal(left.x,-right.x);assert.equal(left.z,right.z);assert.equal(chooseKickFoot(p,{x:-.25,z:.5},{x:0,z:1}),'left');assert.equal(chooseKickFoot(p,{x:.25,z:.5},{x:0,z:1}),'right');});
