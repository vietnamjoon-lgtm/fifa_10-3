...
> **최신 결과: 5,308 → 1,587(1차) → 355(2차, 아래 "2차 수정").**
...
같은 조건(AI 11대11, 60초, seed 5, 15° 기준)으로 측정했습니다.

### 바꾼 것 (src/foot-plant.js, src/motion.js)

1. **B. 발 보정을 다시 걸 때 발이 한 프레임 멈추던 문제**
   - 1차 분석에서 원인을 "소프트 IK 이중 적용"으로 적었는데, **틀렸습니다.**
   - 소프트 IK를 한 번만 적용하도록 고친 뒤(`softenBeyond`: 이번 프레임 발 위치보다 먼 부분만 부드럽게) 다시 재보니, B는 299 → 390으로 오히려 늘었습니다.
   - 다시 추적하니 진짜 원인은 따로 있었습니다. 보정을 다시 걸 때 **직전 프레임 위치**에서 시작해서, 전력질주 중 공중에 있는 발이 한 프레임 멈췄습니다(선수 14, 25.80초: 한 칸 6 cm씩 가던 발이 0 cm). 그동안 골반은 계속 앞으로 가서 무릎이 5° → 37°로 꺾였습니다.
   - 수정: 상태가 바뀌어 보정을 다시 걸 때는 **직전 속도로 한 프레임 더 간 위치**에서 시작합니다(속도는 22 m/s로 제한).
   - 첫 시도에서는 속도 기준으로 다시 걸릴 때도 속도를 이어받게 했습니다. 그랬더니 이어받은 속도가 매 프레임 다시 이어지는 되먹임이 생겨서, 서 있는 선수의 발이 14 m/s로 날아갔습니다(선수 12, 30.29초, 발목 145°).
   - 그래서 속도를 이어받는 건 상태가 바뀔 때만 하도록 했습니다. 속도 기준으로 다시 걸 때 0.05초 동안 잠그는 방식도 비교했는데, 457번으로 더 많아서 쓰지 않았습니다.
   - 소프트 IK를 한 번만 적용하는 수정도 맞는 수정이라 그대로 두었습니다.
2. **A 중 발 고정 쪽: `hipCorrection`이 올라갈 때도 곡선으로.** 내려갈 때처럼 지수 곡선으로 따라가고, 시간 상수는 0.04초입니다. 이전에는 착지 프레임에 한 번에 올라가서 골반이 5.6 cm 떨어졌습니다.
3. **C. 팔 흔들기 켜짐/꺼짐 → 무게값.**
   - `forward > 0.65×speed` 조건을 1-3의 모캡과 같은 무게값(앞으로 가는 비율 0.5~0.8에서 0→1, 0.2초에 걸쳐 따라감)으로 바꿨습니다.
   - 무게값 계산은 두 블록이 같이 쓰도록 `sampleMotion` 위쪽으로 옮겼습니다.

### 결과

| 원인 | 1차 후 | 2차 후 |
|---|---|---|
| A 착지 순간 골반 높이 급변 | 552 | 74 |
| B 발 보정 재계산 | 299 | 34 |
| C 팔 흔들기 켜짐/꺼짐 | 261 | 0 |
| I 걸음 IK 기타 | 279 | 44 |
| E 볼 받기 | 59 | 56 |
...
| 확인 항목 | 값 |
|---|---|
| 상태 전환 순간 무릎 튐 (수정 전 114, 1차 후 259) | **6** |
| 상태 전환 순간 발목 튐 (수정 전 19, 1차 후 78) | 0 |
| 튐 크기 중앙값 / 상위 10% / 최대 | 19.2° / 33.3° / 59.9° |
| 45° 넘는 튐 | 11 |
| 디딤발이 땅에서 뜬 높이 상위 5% | 2.8 cm (수정 전 6.5 cm) |
| 패스 임팩트 순간 발-공 목표 거리 (4회) | 6.8 / 19.9 / 7.9 / 7.0 cm (수정 전과 같음) |
| 스프린트 무릎 한 칸 최대 변화 (선수 17) | 32° → 18° |
| 패스 끝 다리 한 칸 최대 변화 (선수 20) | 71° → 27° |

남은 큰 튐은 대부분 볼 받기(receive-prepare의 발목 50~60°)와 어깨 싸움입니다. 둘 다 공 다루기(7단계)와 물리 반응(9단계) 범위입니다.

### "소프트 IK 한 번만 적용"을 켰을 때와 껐을 때 (2026-09-26)

B의 원래 원인은 이것이 아니었기 때문에, 2차 최종 상태에서 이 수정만 되돌려(소프트 IK를 두 번 적용) 같은 조건으로 다시 쟀습니다.
...
켜는 쪽이 튐 642개를 더 줄입니다. 발이 한 프레임 멈추던 문제를 고친 뒤에는, 보정이 걸린 프레임마다 소프트 IK가 두 번 적용되는 차이가 그대로 무릎 튐으로 드러납니다. 그래서 두 수정이 같이 있어야 효과가 납니다. 이 수정은 유지합니다.
