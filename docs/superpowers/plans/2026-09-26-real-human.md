...
## 범위 변경 (2026-09-26 저녁, 3일 버전)

사용자 결정으로 범위를 줄였다. 아래가 아래쪽 태스크 본문보다 우선한다.

| 항목 | 원래 | 3일 버전 |
|---|---|---|
| LOD | near / mid / far 3단계, 디더링 전환 | **경기용 하나** `player.glb`: 삼각형 ≤ 8,000, 뼈 22개(손가락 없음) |
| 체형 | 3종 모프 | **표준 미드필더형 1종**. 키는 균일 스케일 0.93~1.07 |
| 얼굴 | 8개 | **3개** |
| 피부톤 | 6개 | **3개**: light(young_caucasian_male), brown(caucasian·african 반씩 섞기), dark(young_african_male) |
| 머리 | 짧은 4 + afro + 긴 머리 | **short01, afro01**. 카드 대신 **껍질 한 겹** |
| 눈·치아·속눈썹 | near에 전부 | **눈만 low-poly**. 치아·속눈썹 없음 |
| 유니폼 | 반팔·긴팔·골키퍼 | **반팔 1종** (팀 색 3개 교체, 등번호·이름 텍스처 유지) |
| 손 자세, 골키퍼 유니폼, LOD 전환 | 이번 범위 | 나중에 |
| 검사(태스크 11) | 자세·모캡·극단·발 미끄러짐·fps | **몸 뚫림 + 22명 fps**만 |

진행: 태스크 3부터 이어서 하고, 태스크마다 4방향 렌더를 보여주되 멈추지 않는다(문제나 사용자 결정이 필요할 때만 멈춤). 목표는 2일차 끝까지 `player.glb`와 `human-lab.html`에서 22명 fps 확인. 게임 코드는 건드리지 않고, 연결은 3일차에 따로 한다.

태스크 순서(3일 버전): 3 몸·리그 → 4 얼굴 3 → 5 피부 3톤 굽기 → 6 눈·머리 껍질 → 7 반팔 유니폼 → 8 8천 삼각형으로 줄이기 → 9 내보내기·압축 → 10 뷰어(22명) → 11 뚫림·fps.
