...
export function createFootballBoot(ankle,color){
 ...
 // Batch each rigid part separately: baking across MTP would stop the toes bending.
 const lod=[];let farMaterial;
 for(const group of [foot,toe]){
  const original=group.children.filter(child=>child.isMesh);mergeMeshes(group);
  for(const mesh of original)if(mesh.parent!==group)mesh.geometry.dispose();
  const level=addDistantGeometry(group);
  if(level){farMaterial??=level.far.material.clone();level.far.material=farMaterial;lod.push(level);}
 }
 ankle.userData.toe=toe;ankle.userData.boot=foot;ankle.userData.bootLOD=lod;ankle.userData.contactSurfaces=BOOT_SURFACES;return toe;
}
