...
export function sampleMotion(p={},phase=0,time=0,ball=null,celebrate=false,kinematics={}){
 ...
 if(!p.down&&!p.dive&&!celebrate){
  if(!action&&!p.shield&&forward>speed*.65&&speed>1.5){const cycle=((phase/(Math.PI*2))%1+1)%1;pose.contacts=captureContacts('run',cycle);blendCapture(pose,sampleMocap('run',cycle*mocap.run.duration),smooth((speed-2.2)/2.2)*.82);pose.torso[2]+=clamp(-(kinematics.turn||0)*.018,-.18,.18);}
 ...
 if(speed>.08&&!action&&!p.down&&!p.dive&&!celebrate&&!pose.state.startsWith('receive')&&!(p.turnPlan&&time<p.turnPlan.start+p.turnPlan.duration)&&!(p.interaction&&time<p.interaction.until)){
