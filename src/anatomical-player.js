...
import {uniformLabelProjector} from './uniform-labels.js';
// Cloth rests on the deltoid and collarbone. Inflating both the under-surface
// and the outer shirt by a full centimetre made this area look like padding.
const shoulderCloth=(x,y)=>Math.exp(-Math.pow((Math.abs(x)-.19)/.09,2)-Math.pow((y-1.425)/.10,2));
// Use the closer of the clavicle/yoke and lateral-shoulder cloth fits, so
// overlapping improvements do not stack into an excessively thin shirt.
...
export function createAnatomicalBody(rig,colors){
 ...
 for(let i=0;i<positions.length/3;i++){let arm=0;for(let k=0;k<4;k++){const b=anatomy.body.skinIndices[i*4+k];if(b>=3&&b<=6)arm+=anatomy.body.skinWeights[i*4+k];}armWeights.push(arm);const x=positions[i*3],y=positions[i*3+1],neck=1.42+.13*Math.min(1,Math.abs(x)/.15),top=neck*(1-arm)+1.23*arm,shirt=(1-arm)*THREE.MathUtils.smoothstep(y,.95,.97)*(1-THREE.MathUtils.smoothstep(y,neck-.012,neck))+arm*THREE.MathUtils.smoothstep(y,1.225,1.235),shorts=THREE.MathUtils.smoothstep(y,.70,.72)*(1-THREE.MathUtils.smoothstep(y,.96,.975))*(1-arm),yoke=Math.exp(-Math.pow((y-1.40)/.07,2)),thickness=shirt*Math.min(.012*(1-.75*shoulderCloth(x,y)),.012-.007*yoke)+shorts*.014;positions[i*3]+=normals.getX(i)*thickness;positions[i*3+1]+=normals.getY(i)*thickness*.35;positions[i*3+2]+=normals.getZ(i)*thickness;}
 ...
 float fabric=max(shirt,max(shorts,sock));
 roughnessFactor=mix(.65,.91,fabric);
 #ifdef CLOSE_UNIFORM
 // Sub-pixel weave fades out with screen derivatives instead of shimmering.
 vec2 threads=vWardrobe.xy*1800.0;
 float weaveVisibility=1.0-smoothstep(.5,2.0,max(fwidth(threads.x),fwidth(threads.y)));
 float weave=sin(threads.x)*sin(threads.y)*weaveVisibility;
 diffuseColor.rgb*=1.0+fabric*weave*.018;
 roughnessFactor+=fabric*weave*.025;
 #endif
 `);shader.fragmentShader='#define CLOSE_UNIFORM\n'+shader.fragmentShader;dualQuaternionShader(shader);};
 ...
 const rootInverse=rig.root.matrixWorld.clone().invert(),projectLabel=uniformLabelProjector(clothing);
 rig.details=(rig.details||[]).map(label=>{const geometry=projectLabel(label.geometry.clone().applyMatrix4(new THREE.Matrix4().multiplyMatrices(rootInverse,label.matrixWorld)));label.material.onBeforeCompile=dualQuaternionShader;label.material.customProgramCacheKey=()=> 'human-dqs-label-v2';label.material.polygonOffset=true;label.material.polygonOffsetFactor=-1;label.material.polygonOffsetUnits=-1;const replacement=new THREE.SkinnedMesh(geometry,label.material);replacement.frustumCulled=false;rig.root.add(replacement);replacement.bind(skeleton,mesh.bindMatrix);label.removeFromParent();label.geometry.dispose();return replacement;});
 const update=skeleton.update.bind(skeleton);skeleton.update=()=>{updateHumanDeformation(rig,rig.animationPlayer,rig.animationTime);updateHumanCorrectives(rig,[mesh,garment,...rig.details]);update();};
 ...
 const farMat=mat.clone();farMat.onBeforeCompile=shader=>{const original=shader.vertexShader;mat.onBeforeCompile(shader);shader.fragmentShader=shader.fragmentShader.replace('#define CLOSE_UNIFORM\n','');shader.vertexShader=original.replace('#include <common>','#include <common>\nattribute vec3 wardrobePosition; attribute float wardrobeArm; varying vec3 vWardrobe; varying float vArm;').replace('#include <begin_vertex>','#include <begin_vertex>\nvWardrobe=wardrobePosition;vArm=wardrobeArm;');};farMat.customProgramCacheKey=()=> 'human-lbs-wardrobe-v2';
