...
export function createAnatomicalBody(rig,colors){
 ...
 for(let i=0;i<clothPositions.count;i++){const y=wardrobePositions[i*3+1],x=wardrobePositions[i*3],arm=armWeights[i],hem=Math.exp(-(((y-.966)/.035)**2)),cuff=Math.exp(-(((y-(arm>.5?1.23:.71))/.028)**2)),shoulder=shoulderCloth(x,y),offset=.007*(1-.45*shoulder)+.012*hem+.009*cuff+.0025*(1-.6*shoulder)*Math.sin(x*95+y*42)*Math.sin(y*79);for(let axis=0;axis<3;axis++)clothPositions.array[i*3+axis]+=clothNormals.array[i*3+axis]*offset;}
