...
import {createPhysics,detectGoal} from './physics.js';
...
import {ASSIST,footPosition,shotTarget,groundPassSpeed,passTarget,setKickTarget,dribbleTouch,cushionFirstTouch} from './assists.js';
...
export class Match{
 ...
 if(!aim){
  ...
  if(type==='shoot'){
   ...
   const follow=followPassEnabled(this,p.team);const pass=mode==='manual'&&!follow?null:choosePass(this,p,hasInput?axis:null,type)||(follow?choosePass(this,p,null,type):null);
 ...
 contactKick(p,a){const b=this.physics.ball.position,foot=footPosition(p);const reachable=a.aerial?distance(p,b)<.95&&b.y>.4&&b.y<2.25:distance(foot,b)<=ASSIST.contactRadius&&b.y<=.65;if(!reachable){a.missed=true;this.emit('miss',{player:p});return;}
 ...
 if(a.receiver?.active){a.target=passTarget(this,p,a.receiver,a.type);if(a.assistOffset){a.target.x+=a.assistOffset.x;a.target.z+=a.assistOffset.z;}}
 if(a.target)setKickTarget(a,b,a.target);
 const pressure=Math.min(...this.players.filter(q=>q.team!==p.team&&q.active).map(q=>distance(p,q)),20);let speed,lift,curve=a.curve||0;
 ...
 else {const lob=a.type==='lob'||a.lob;speed=lob?clamp(9+a.distance*.46,11,30):groundPassSpeed(a.distance,a.type);lift=lob?clamp(a.distance*.23,5,11):a.type==='through'?.25:.08;if(a.driven)speed=Math.min(38,speed*1.3);if(a.groundCross)lift=.1;if(a.lowCross)lift=2;if(a.bounce)lift=3.3;if(a.early){speed*=1.1;lift*=1.05;}speed*=.96+.04*kickSkill(p,a);this.stats.passes[p.team]++;}
 const setFlight=a.type==='shoot'?setpieceFlight(a.flightStyle,p,a,b):null;if(setFlight){speed=setFlight.speed;lift=setFlight.lift;curve=setFlight.curve;}else if(a.type==='shoot'&&a.flair&&a.power>=.4&&!a.aerial){curve=-14*(a.foot==='left'?-1:1);}
 if(a.aerial){speed*=b.y>1.2?.72:.92;lift=a.low?-1:Math.min(lift,2);}
 const assistKey=a.type==='lob'?'crossAssist':a.type==='through'?'throughAssist':'passAssist',assisted=a.type!=='shoot'&&a.receiver&&(this.settings[assistKey]||'auto')!=='manual';const error=a.type!=='shoot'&&a.receiver&&followPassEnabled(this,p.team)?0:kickError(p,a,pressure)*(assisted?((this.settings[assistKey]||'auto')==='auto'?.48:.75):1);const angle=(this.random()-.5)*error;const dx=a.aim.x*Math.cos(angle)-a.aim.z*Math.sin(angle),dz=a.aim.x*Math.sin(angle)+a.aim.z*Math.cos(angle);
 ...
 if(!a.hit&&this.owner===p)dribbleTouch(this,p,true);
 ...
 if(this.setPiece?.kind==='throw')return;
 if(this.heldBy){
 ...
 if(this.owner&&(!this.owner.active||distance(this.owner,b)>ASSIST.releaseRadius||b.y>.85||this.owner.down>0))this.owner=null;
 ...
 autoRestart(s){if(s.kind==='throw')throwIn(this,'pass');else this.queueKick(s.taker,s.kind==='penalty'?'shoot':'pass',.65);}
 ...
 if(!p.active)return;if(p.intent){const intent=p.intent;if(this.time>intent.expires||p!==this.controlled||p.down>0)p.intent=null;else if(distance(p,this.physics.ball.position)<ASSIST.touchRadius&&!p.action){const incoming=this.physics.ball.velocity.length();if(incoming<15||this.owner===p){const saved=this.input.axis;this.input.axis=intent.axis;if(this.queueKick(p,intent.type,intent.power,null,null,intent.options)){p.action.inputTime=intent.inputTime;p.intent=null;}this.input.axis=saved;}}}p.closeControl=false;p.shield=false;p.autoDefending=false;let axis,sprint=false,defend=false;if(p===this.controlled&&!this.autoplay){axis=this.carryInput&&this.time<this.carryInput.until?this.carryInput.axis:input.axis||{x:0,z:0};sprint=this.settings.analogSprint&&input.sprintAmount!==undefined?input.sprintAmount:input.sprint;defend=input.defend;p.closeControl=input.closeControl&&this.owner===p;p.shield=input.shield&&this.owner===p;
 ...
 const ball=this.physics.ball,previous={x:ball.position.x,y:ball.position.y,z:ball.position.z};const t=performance.now();guidePass(this,dt);if(!this.heldBy)this.physics.step(dt);this.physicsMs=performance.now()-t;resolveBodyContacts(this,previous);if(this.state!=="playing")return;
 const goal=detectGoal(previous,ball.position);if(goal!==-1){const team=this.half===1?goal:1-goal;
 if(this.restartOrigin&&['throw','indirect'].includes(this.restartOrigin.kind)){const own=team!==this.restartOrigin.team,end=Math.sign(ball.position.x);this.beginRestart({kind:own?'corner':'goalkick',team:own?team:1-team,x:end*(own?52:47),z:own?33.5:5,label:own?'코너킥':'직접 득점 불가 · 골킥'});return;}
