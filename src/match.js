...
export function kickFacing(p,a){const aim=Math.atan2(a.aim.x,a.aim.z),speed=Math.hypot(p.vx,p.vz);if(!a.braking||speed<.8)return aim;
 const heading=Math.atan2(p.vx,p.vz),diff=Math.atan2(Math.sin(heading-aim),Math.cos(heading-aim));
 return aim+(a.hit?diff*clamp(speed/4,0,.6):Math.sign(diff)*Math.min(Math.abs(diff)*.3,.52));}
export class Match{
 ...
 this.approachKick(p,p.action);
 this.receiving=null;
 return true;
 }
 // A kick across the run is planted: the kicker brakes into it, and the set-up touch rolls the ball to where the kicking
 // foot will meet it. Without it a sprinting crosser turned toward the box while the ball ran on and the kick missed.
 approachKick(p,a){const b=this.physics.ball.position,speed=Math.hypot(p.vx,p.vz),heading=speed>.5?Math.atan2(p.vx,p.vz):p.yaw,aimYaw=Math.atan2(a.aim.x,a.aim.z);
  a.turn=Math.abs(Math.atan2(Math.sin(aimYaw-heading),Math.cos(aimYaw-heading)));a.entrySpeed=speed;a.plantSpeed=speed*(1-KICK_PLANT.brake*clamp((a.turn-KICK_PLANT.turn)/1.1,0,1));a.braking=speed>=KICK_PLANT.minSpeed&&a.turn>=KICK_PLANT.turn;
  if(this.owner!==p||a.aerial||b.y>.3||!a.braking||this.setPiece)return;
  const t=Math.max(.12,a.contactAt),hx=p.vx/speed,hz=p.vz/speed,side=footSign(a.foot)*.11;let travel=0,v=speed;
  for(let i=0;i<12;i++){const s=(i+.5)/12;v=Math.max(plantSpeedAt(a,s*t),v-KICK_PLANT.deceleration*t/12);travel+=v*t/12;}
  const tx=p.x+hx*travel+a.aim.x*.48+a.aim.z*side,tz=p.z+hz*travel+a.aim.z*.48-a.aim.x*side,dx=tx-b.x,dz=tz-b.z,need=Math.hypot(dx,dz)/t;
  if(need>Math.max(speed,this.physics.ball.velocity.length())+1.5)return;
  this.physics.kick({x:dx,z:dz},need,.01);this.lastTouch=p;this.lastTouchTeam=p.team;this.lastTouchKind='touch';p.touchCooldown=t;p.dribbleTouch=.16;a.planted=true;
 }
 ...
 a.contactTime=this.time;a.ballReleaseTime=this.time;a.actualTarget={x:b.x,y:b.y,z:b.z};a.contactTarget={...a.actualTarget};a.nextActionAllowed=this.time+(a.recovery||.28);
 ...
 // The aim commits independently of the contact location. The moving ball remains the foot's
 // target until release, when contactKick freezes the exact departure point for local/online poses.
 if(!a.hit){const b=this.physics.ball.position;a.contactTarget={x:b.x,y:b.y,z:b.z};if(!a.commitTime){if(a.target)setKickTarget(a,b,a.target);if(a.elapsed>=a.commitAt){a.commitTime=this.time;a.committedTarget={...a.contactTarget};}}}
 p.yaw=turnToward(p.yaw,kickFacing(p,a),a.hit?dt*6:Math.max(dt*5,Math.abs(Math.atan2(Math.sin(kickFacing(p,a)-p.yaw),Math.cos(kickFacing(p,a)-p.yaw)))*dt/Math.max(dt,a.contactAt-a.elapsed)));
 ...
 move(p,axis,sprint,defend,dt){if(this.owner===p&&!this.heldBy&&!p.action){axis=dribbleSteer(this,p,axis);if(p.dribbleChase||p.pendingKick)sprint=true;}else{p.dribbleAim=null;p.dribbleStop=p.dribbleChase=false;p.sprintHeld=false;p.knockFresh=false;}if(!p.action&&!p.turnPlan&&Math.hypot(p.vx,p.vz)>2&&Math.hypot(axis.x,axis.z)>.5){const angle=Math.atan2(Math.sin(Math.atan2(axis.x,axis.z)-p.yaw),Math.cos(Math.atan2(axis.x,axis.z)-p.yaw));if(Math.abs(angle)>1.4)p.turnPlan={start:this.time,duration:.24+Math.abs(angle)*.035,angle,foot:chooseKickFoot(p,this.physics.ball.position,axis)};}if(p.turnPlan&&this.time>p.turnPlan.start+p.turnPlan.duration)p.turnPlan=null;let n=Math.hypot(axis.x,axis.z),jog=jogSpeed(p),speed=(sprint?jog+(sprintSpeed(p)-jog)*(typeof sprint==='number'?sprint:1):jog)*(defend?(p.jockey?.92:p.autoDefending&&distance(p,this.physics.ball.position)>3.4?.93:.68):1)*(this.owner===p?.96:1)*(p.closeControl?.72:1);if(p.action?.hit)speed*=.65;/* A dribbler about to kick takes a short burst onto a ball knocked ahead. */if(this.owner===p&&(p.pendingKick||p.action?.aim&&!p.action.hit))speed*=ASSIST.kickBurst;if(p.action?.aim&&!p.action.hit&&p.action.braking)speed=Math.min(speed,plantSpeedAt(p.action,p.action.elapsed)+.25);if(['drag-back','drag-to-heel','ball-roll'].includes(p.action?.skill))speed=Math.min(speed,2.4);if(p.action?.type==='tackle')speed*=.65;if(p.speedCap>0&&!this.isHumanControlled(p))speed=Math.min(speed,p.speedCap);if(p.down>0)speed=0;
 if(p.role==='GK'&&!this.isHumanControlled(p))speed=Math.min(speed,p.dive>0?.65:KEEPER.moveSpeed);const previousSpeed=Math.hypot(p.vx,p.vz),previousYaw=p.yaw,profile=movementProfile(p,previousSpeed,axis);profile.acceleration*=gameplayValue(this,'acceleration');profile.braking*=gameplayValue(this,'braking');profile.turn*=gameplayValue(this,'turnResponse');const align=previousSpeed>.2&&n>.05?(p.vx*axis.x+p.vz*axis.z)/(previousSpeed*n):1;speed*=1-clamp((1-align)*.5,0,1)*clamp(previousSpeed/8,0,1)*.28;const targetX=axis.x*speed,targetZ=axis.z*speed,planting=p.action?.aim&&!p.action.hit&&p.action.braking,acc=planting?profile.braking*1.35:n>.05?(align<-.1?profile.braking:profile.acceleration):profile.braking;let dx=targetX-p.vx,dz=targetZ-p.vz,change=Math.hypot(dx,dz),max=acc*dt*(p===this.controlled&&this.settings.defenceAssist&&this.owner?.team!==p.team&&defend?1.3:1);if(change>max){dx*=max/change;dz*=max/change}p.vx+=dx;p.vz+=dz;
 ...
 }else {let x=p.target.x-p.x,z=p.target.z-p.z,n=Math.hypot(x,z);axis=n>.25?{x:x/n*Math.min(n/1.3,1),z:z/n*Math.min(n/1.3,1)}:{x:0,z:0};sprint=p.sprinting;defend=!!p.jockey&&!sprint;}
