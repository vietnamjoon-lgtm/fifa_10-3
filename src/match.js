...
import {FIELD,TUNING,roster,clamp,distance,turnToward,jogSpeed,sprintSpeed} from './config.js';
import {updateTeamAI,choosePass,keeperTarget} from './ai.js';
import {boundaryRestart,offsideSnapshot} from './rules.js';
import {ASSIST,footPosition,shotTarget,groundPassSpeed,passTarget,setKickTarget,dribbleTouch,cushionFirstTouch,possessionRadius,dribbleSteer,controlReach,kickLunge,kickInReach,settlePosePulls,applyBallDrag} from './assists.js';
...
export class Match{
 ...
 move(p,axis,sprint,defend,dt){if(this.owner===p&&!this.heldBy&&!p.action){axis=dribbleSteer(this,p,axis);if(p.dribbleChase||p.pendingKick)sprint=true;}else{p.dribbleAim=null;p.dribbleStop=p.dribbleChase=p.agile=false;p.sprintHeld=false;p.knockFresh=false;p.cutTo=null;}if(!p.action&&!p.turnPlan&&Math.hypot(p.vx,p.vz)>2&&Math.hypot(axis.x,axis.z)>.5){const angle=Math.atan2(Math.sin(Math.atan2(axis.x,axis.z)-p.yaw),Math.cos(Math.atan2(axis.x,axis.z)-p.yaw));if(Math.abs(angle)>1.4)p.turnPlan={start:this.time,duration:.24+Math.abs(angle)*.035,angle,foot:chooseKickFoot(p,this.physics.ball.position,axis)};}if(p.turnPlan&&this.time>p.turnPlan.start+p.turnPlan.duration)p.turnPlan=null;let n=Math.hypot(axis.x,axis.z),jog=jogSpeed(p),speed=(sprint?jog+(sprintSpeed(p)-jog)*(typeof sprint==='number'?sprint:1):jog)*(defend?(p.autoDefending&&distance(p,this.physics.ball.position)>3.4?.93:.68):1)*(this.owner===p?.96:1)*(p.closeControl?.72:this.owner===p&&p.agile?ASSIST.agilePace:1);if(p.action?.hit)speed*=.65;/* A dribbler about to kick takes a short burst onto a ball knocked ahead. */if(this.owner===p&&(p.pendingKick||p.action?.aim&&!p.action.hit))speed*=ASSIST.kickBurst;if(p.action?.type==='feint'&&p.action.pace!=null)speed=Math.min(speed,p.action.pace);else if(['drag-back','drag-to-heel','ball-roll'].includes(p.action?.skill))speed=Math.min(speed,2.4);if(p.action?.type==='tackle')speed*=.65;if(p.down>0)speed=0;
