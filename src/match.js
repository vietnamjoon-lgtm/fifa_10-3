...
import {ASSIST,footPosition,shotTarget,groundPassSpeed,passTarget,setKickTarget,dribbleTouch,cushionFirstTouch,possessionRadius,dribbleSteer,controlReach,kickLunge,settlePosePulls,applyBallDrag} from './assists.js';
...
export class Match{
 ...
 for(const p of this.players){const d=this.direction(p.team);p.x=p.homeX*d;p.z=p.homeZ;p.vx=0;p.vz=0;p.yaw=d*Math.PI/2;p.action=null;p.intent=null;p.motionPhase=0;p.motionAcceleration=0;p.motionTurn=0;p.turnDrive=0;p.receiveUntil=0;p.receive=null;p.receivePrep=null;p.autoDefending=false;p.autoDefendSince=null;p.dribblePose=null;p.touchWindup=null;p.strideNext=null;p.lastAim=null;p.aimTurnAt=undefined;p.agile=false;p.strideWarp=1;p.cutTouch=null;p.cutTo=null;p.ballDrag=null;p.dragRestUntil=null;p.turnPlan=null;p.keeperMotion=null;p.keeperRead=null;p.chargeContactUntil=0;p.diveDuration=0;p.diveHeight=0;p.interaction=null;p.wallHoldUntil=0;p.cooldown=.5;p.touchCooldown=0;p.possessedAt=-1;p.dive=0;p.down=0;p.active=!p.sentOff&&(!this.practice||p.id===this.settings.userTeam*11+9);p.target={x:p.x,z:p.z};}
 ...
 if(this.owner===p&&!options.deferred&&distance(p,ball)>ASSIST.kickStart){p.pendingKick={args:[type,power,aim,receiver,{...options,deferred:true}],axis:{...(this.input.axis||{x:0,z:0})},expires:this.time+ASSIST.pendingKick};return true;}
 ...
 if(a.type==='feint'){if(a.move){const u=Math.min(1,a.elapsed/a.duration),e=u*u*(3-2*u);if(a.turn)p.yaw=a.yaw0+a.turn*e;a.liveAxis=this.isHumanControlled(p)?this.inputForTeam(p.team).axis:null;}if(a.events){for(const event of a.events){if(event.done||a.elapsed<event.at)continue;event.done=true;event.settle=true;const b=this.physics.ball.position;if(distance(p,b)>(a.air?1.2:.95)||b.y>(a.air?1.3:.4)||this.owner&&this.owner!==p&&!(a.air&&this.lastTouch===p)){a.failed=true;continue;}const impulse=skillImpulse(p,a,event.stage??(event.at>.12?1:0));this.physics.kick(impulse.aim,impulse.speed,impulse.lift);p.touchCooldown=.12;this.lastTouch=p;this.lastTouchTeam=p.team;this.lastTouchKind='touch';}if(a.elapsed>(a.duration||.42))p.action=null;}else{if(this.owner===p&&!this.setPiece)dribbleTouch(this,p);if(a.elapsed>.28)p.action=null;}return;}
 ...
 if(a.elapsed>=a.contactAt&&!a.hit){const aligned=Math.sin(p.yaw)*a.aim.x+Math.cos(p.yaw)*a.aim.z>.82,reach=a.aerial?distance(p,this.physics.ball.position)<.95:distance(footPosition(p),this.physics.ball.position)<ASSIST.contactRadius+kickLunge(p,a,this.physics.ball.position);if(aligned&&reach||a.elapsed>.62){a.hit=true;a.contactAt=a.elapsed;this.contactKick(p,a);}else a.contactAt+=dt;}
 ...
 move(p,axis,sprint,defend,dt){if(this.owner===p&&!this.heldBy&&!p.action){axis=dribbleSteer(this,p,axis);if(p.dribbleChase||p.pendingKick)sprint=true;}else{p.dribbleAim=null;p.dribbleStop=p.dribbleChase=p.agile=false;p.cutTo=null;}if(!p.action&&!p.turnPlan&&Math.hypot(p.vx,p.vz)>2&&Math.hypot(axis.x,axis.z)>.5){const angle=Math.atan2(Math.sin(Math.atan2(axis.x,axis.z)-p.yaw),Math.cos(Math.atan2(axis.x,axis.z)-p.yaw));if(Math.abs(angle)>1.4)p.turnPlan={start:this.time,duration:.24+Math.abs(angle)*.035,angle,foot:chooseKickFoot(p,this.physics.ball.position,axis)};}if(p.turnPlan&&this.time>p.turnPlan.start+p.turnPlan.duration)p.turnPlan=null;let n=Math.hypot(axis.x,axis.z),speed=(sprint?TUNING.jog+(p.pace*(.75+p.stamina*.25)-TUNING.jog)*(typeof sprint==='number'?sprint:1):TUNING.jog)*(defend?(p.autoDefending&&distance(p,this.physics.ball.position)>3.4?.93:.68):1)*(this.owner===p?.93:1)*(p.closeControl?.72:this.owner===p&&p.agile?ASSIST.agilePace:1);if(p.action?.hit)speed*=.65;if(p.action?.type==='feint'&&p.action.pace!=null)speed=Math.min(speed,p.action.pace);else if(['drag-back','drag-to-heel','ball-roll'].includes(p.action?.skill))speed=Math.min(speed,2.4);if(p.action?.type==='tackle')speed*=.65;if(p.down>0)speed=0;
 if(p.role==='GK'&&!this.isHumanControlled(p))speed=Math.min(speed,p.dive>0?.65:KEEPER.moveSpeed);const previousSpeed=Math.hypot(p.vx,p.vz),previousYaw=p.yaw,profile=movementProfile(p,previousSpeed,axis);profile.acceleration*=gameplayValue(this,'acceleration');profile.braking*=gameplayValue(this,'braking');profile.turn*=gameplayValue(this,'turnResponse');const align=previousSpeed>.2&&n>.05?(p.vx*axis.x+p.vz*axis.z)/(previousSpeed*n):1;speed*=1-clamp((1-align)*.5,0,1)*clamp(previousSpeed/8,0,1)*.28;const targetX=axis.x*speed,targetZ=axis.z*speed,acc=n>.05?(align<-.1?profile.braking:profile.acceleration):profile.braking;const cutTo=this.owner===p&&!p.action&&p.cutTo&&this.time-p.cutTo.at<ASSIST.cutWait&&(!p.dribbleAim||p.dribbleAim.x*p.cutTo.x+p.dribbleAim.z*p.cutTo.z>Math.cos(ASSIST.cutMin))?p.cutTo:null;if(!cutTo)p.cutTo=null;
 // A cut (see startCut in assists.js): the run turns onto the ball's new line at a fixed rate, easing off a little, and
 // hands back to the usual steering once on it.
 if(cutTo&&previousSpeed>.5){const bp=this.physics.ball.position,gx=bp.x+cutTo.x*ASSIST.cutLead-p.x,gz=bp.z+cutTo.z*ASSIST.cutLead-p.z,heading=Math.atan2(p.vx,p.vz),goal=Math.atan2(gx,gz),off=Math.atan2(Math.sin(goal-heading),Math.cos(goal-heading)),dir=heading+clamp(off,-ASSIST.cutRate*dt,ASSIST.cutRate*dt),next=previousSpeed+clamp(speed*(1-(1-ASSIST.cutPace)*Math.min(1,Math.abs(off)/ASSIST.cutMin))-previousSpeed,-profile.braking*dt,profile.acceleration*dt);p.vx=Math.sin(dir)*next;p.vz=Math.cos(dir)*next;p.cutting=true;if(Math.abs(off)<ASSIST.cutAlign&&Math.abs((bp.x-p.x)*cutTo.z-(bp.z-p.z)*cutTo.x)<ASSIST.cutLane)p.cutTo=null;}
 else{let dx=targetX-p.vx,dz=targetZ-p.vz,change=Math.hypot(dx,dz),max=acc*dt*(p===this.controlled&&this.settings.defenceAssist&&this.owner?.team!==p.team&&defend?1.3:1);if(change>max){dx*=max/change;dz*=max/change}p.vx+=dx;p.vz+=dz;}
 ...
 const current=Math.hypot(p.vx,p.vz);if(current>.2&&!p.action){const yaw=defend?Math.atan2(this.physics.ball.position.x-p.x,this.physics.ball.position.z-p.z):align<.1&&n>.1?Math.atan2(axis.x,axis.z):Math.atan2(p.vx,p.vz);p.yaw=turnToward(p.yaw,yaw,Math.max(profile.turn,p.cutting?ASSIST.cutRate:0)*dt);}p.cutting=false;
 p.motionAcceleration=(current-previousSpeed)/dt;p.motionTurn=Math.atan2(Math.sin(p.yaw-previousYaw),Math.cos(p.yaw-previousYaw))/dt;{const want=Math.min(2.5,Math.abs(p.motionTurn)*.3);p.turnDrive=(p.turnDrive||0)+(want-(p.turnDrive||0))*(1-Math.exp(-dt/.15));}p.motionPhase=(p.motionPhase||0)+dt*locomotionCadence(Math.max(current,p.turnDrive),p.motionStyle,p)*(this.owner===p&&!p.action?p.strideWarp||1:1);p.defending=!!defend;
 ...
 if(!p.active)return;
 if(p.pendingKick){const k=p.pendingKick;if(this.time>k.expires||this.owner!==p||p.down>0||p.action)p.pendingKick=null;else if(distance(p,this.physics.ball.position)<=ASSIST.kickStart){p.pendingKick=null;const saved=this.input.axis;this.input.axis=k.axis;this.queueKick(p,...k.args);this.input.axis=saved;}}
 ...
 const ball=this.physics.ball,previous={x:ball.position.x,y:ball.position.y,z:ball.position.z};const t=performance.now();guidePass(this,dt);applyBallDrag(this);if(!this.heldBy&&!this.setPiece)this.physics.step(dt);this.physicsMs=performance.now()-t;settlePosePulls(this);resolveBodyContacts(this,previous);if(this.state!=="playing")return;
