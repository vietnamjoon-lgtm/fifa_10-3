...
export class Match{
 ...
 move(p,axis,sprint,defend,dt){if(this.owner===p&&!this.heldBy&&!p.action){axis=dribbleSteer(this,p,axis);if(p.dribbleChase||p.pendingKick)sprint=true;}else{p.dribbleAim=null;p.dribbleStop=p.dribbleChase=false;}if(!p.action&&!p.turnPlan&&Math.hypot(p.vx,p.vz)>2&&Math.hypot(axis.x,axis.z)>.5){const angle=Math.atan2(Math.sin(Math.atan2(axis.x,axis.z)-p.yaw),Math.cos(Math.atan2(axis.x,axis.z)-p.yaw));if(Math.abs(angle)>1.4)p.turnPlan={start:this.time,duration:.24+Math.abs(angle)*.035,angle,foot:chooseKickFoot(p,this.physics.ball.position,axis)};}if(p.turnPlan&&this.time>p.turnPlan.start+p.turnPlan.duration)p.turnPlan=null;let n=Math.hypot(axis.x,axis.z),jog=jogSpeed(p),speed=(sprint?jog+Math.max(0,p.pace*(.75+p.stamina*.25)-jog)*(typeof sprint==='number'?sprint:1):jog)*(defend?(p.autoDefending&&distance(p,this.physics.ball.position)>3.4?.93:.68):1)*(this.owner===p?.96:1)*(p.closeControl?.72:1);if(p.action?.hit)speed*=.65;if(['drag-back','drag-to-heel','ball-roll'].includes(p.action?.skill))speed=Math.min(speed,2.4);if(p.action?.type==='tackle')speed*=.65;if(p.down>0)speed=0;
