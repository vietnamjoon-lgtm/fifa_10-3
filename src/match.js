...
import {ASSIST,footPosition,shotTarget,groundPassSpeed,passTarget,setKickTarget,dribbleTouch,cushionFirstTouch,possessionRadius,dribbleSteer,controlReach,kickLunge} from './assists.js';
...
export class Match{
 ...
 contactKick(p,a){const b=this.physics.ball.position,foot=footPosition(p);const reachable=a.aerial?distance(p,b)<.95&&b.y>.4&&b.y<2.25:distance(foot,b)<=ASSIST.contactRadius+kickLunge(p,a,b)&&b.y<=.65;if(!reachable){a.missed=true;this.emit('miss',{player:p});return;}
 ...
 let aim=a.aim;if(curve&&a.flightStyle!=='knuckle'&&lift>1&&a.distance>1){const n=Math.hypot(aim.x,aim.z)||1,x=aim.x/n,z=aim.z/n,k=Math.sign(curve)*curveDrift(speed,curve,a.distance)/a.distance;aim={x:x-k*z,z:z+k*x};}
