...
export function prepareReception(match,p,dt,input){
 ...
 const pace=Math.hypot(v.x,v.z)||1;p.receivePrep={start,until:match.time+.1,eta:target.time,foot,kind,weight,target:{x:target.x,y:target.y,z:target.z},incoming:{x:v.x/pace,z:v.z/pace}};
