...
// Physical flight prediction for loose-ball collection and optional reception assistance.
...
export function collectionMovement(match,p,axis,input={}){
 ...
 // Idle input or a stick aimed toward the ball can assist the approach. Deliberate sideways or
 // away input keeps control of the receiver too; assistance must not reverse the user's movement.
 const stick=Math.hypot(axis.x,axis.z);
 if(stick>.12&&(n<=.38||(axis.x*dx+axis.z*dz)/(stick*n)<Math.cos(75*Math.PI/180)))return null;
